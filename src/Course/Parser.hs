...
data ParseResult a =
  ...
  | Result Input a -- produces: Result <remainder> <value> .. e.g. "abcd" -> Result "bcd" "a"
  deriving Eq
...
valueParser ::
  ...
  -> Parser a
valueParser a =
  P (\input -> Result input a) -- does nothing with the input, just returns (Result Input a)
...
mapParser ::
  ...
  -> Parser b
mapParser f (P p) =
  -- (P k) pattern match allows us to pull apart (Parser a)
  -- k has type Input -> (ParseResult a)
  -- .. therefore (k Input) has type (ParseResult a)
  -- .. our goal is to get to :: ParseResult b
  --
  -- but how?
  --
...
-- bindParser :: (a -> f b) -> f a -> f b where f is Parser
-- ... but (=<<) :: Bind f => (a -> f b) -> f a -> f b
--
bindParser ::
  (a -> Parser b)
  -> Parser a
...
(|||) (P p1) (P p2) =
  P (\input -> case p1 input of
                 ErrorResult _ -> p2 input -- try parser p2
                 Result j a -> Result j a  -- otherwise just return the result of parser p1
                 --r@Result{} -> r         -- equivalent to the above line, without naming the args of Result; binds to 'r'
                 )
...
list ::
  ...
  -> Parser (List a)
list p =
  -- parse non-empty list OR parser that always produces Nil
...
list1 p =
  -- approach:
  -- run parser p to produce a (once), then (bind) run (list p) to produce
  -- a list of as ...  then return a:.as
  ...
  -- solution 2:
  -- another way to do this is do notation:
  -- (note: pure == valueParser here ...)
  -- do a <- p
  --    as <- list p
  --    pure (a:.as)
...
satisfy t =
  -- solution 1:
  -- character >>= \c -> if t c
  --                     then valueParser c           -- success case
  --                     else unexpectedCharParser c  -- failure case
...
natural ::
  Parser Int
natural =
  -- run 1 or more digits to produce digits
  -- if (read ds) gives Full (Int) then yay else no
...
firstNameParser ::
  Parser Chars
firstNameParser =
  do u  <- upper       -- one upper-case character
     ls <- list lower  -- list --> zero or more lower-case characters. (list1 --> 1 or more)
     pure (u:.ls)
...
surnameParser =
  ...
  -- using do notation
  do u  <- upper                -- one upper-case character
     ls <- thisMany 5 $ lower   -- 5 lower case characters
     rm <- list lower           -- zero or  more lower-case characters
     pure (u :. ls ++ rm)       -- --> uppercase :. 5 lowercase ++ remainder

  -- solution 2:
  -- a solution using applicative syntax:
  -- (\c d e -> c :. d ++ e) <$> upper <*> thisMany 5 lower <*> list lower

  -- solution 3:
  -- same as above, but using lift3
...
phoneBodyParser ::
  Parser Chars
phoneBodyParser =
  -- list (zero or more) of: digits OR dots OR hyphens
  list (digit ||| is '.' ||| is '-')
...
phoneParser =
  -- solution 1:
  -- using do notation
  -- do d <- digit           -- d = one digit
  --    r <- phoneBodyParser -- r = a phoneBody (remainder)
  --    is '#'               -- a hash
  --    pure (d:.r)
  ...
  -- <* in above throws the r-arg away ..
  -- (remember it as "arrow points towards the thing you want to keep!")
  --
  -- larg <* rarg --> keeps larg
  -- larg *> rarg --> keeps rarg
